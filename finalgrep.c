...
void print(void) { unsigned int *a1; nonzero(); a1 = addr1;
	do { if (listn) { count = a1-zero; putd(); putchr('\t'); } puts(_getline(*a1++)); } while (a1 <= addr2);
...
void filename(int comm) { char *p1, *p2; int c; count = 0; c = getchr();
	if (c=='\n' || c==EOF) {
		p1 = savedfile; if (*p1==0 && comm!='f') { error(Q); } p2 = file; while (*p2++ = *p1++) { } return;
	}
	if (c!=' ') { error(Q); }
	while ((c = getchr()) == ' ') { } if (c=='\n') { error(Q); } p1 = file;
	do {
...
void onintr(int n) { signal(SIGINT, onintr); putchr('\n'); lastc = '\n'; error(Q); }
void onhup(int n) {
	signal(SIGINT, SIG_IGN); signal(SIGHUP, SIG_IGN);
	if (dol > zero) { addr1 = zero+1; addr2 = dol; io = creat("ed.hup", 0600); if (io > 0) { putfile(); } }
	fchange = 0; quit(0);
}
void error(char *s) { int c; listf = 0; listn = 0; putchr('?'); puts(s); count = 0;
	lseek(0, (long)0, 2); pflag = 0; if (globp) { lastc = '\n'; } globp = 0; peekc = lastc;
...
int getchr(void) {  char c;
	...
	if (globp) {
		if ((lastc = *globp++) != 0) { 	return(lastc); } globp = 0; return(EOF); }
...
void grepline(void){ getchr(); for (int i = 0; i < 50; ++i){ putchr('-'); } putchr('\n'); }
void greperror(char c) {  getchr();  /* throw away '\n' */
  snprintf(grepbuf, sizeof(grepbuf), "\'%c\' is a non-grep command", c);  puts(grepbuf);  }
void putfile(void) { unsigned int *a1; int n; char *fp, *lp; int nib; nib = BLKSIZE; fp = genbuf; a1 = addr1;
	do {
		lp = _getline(*a1++);
		for (;;) {
			if (--nib < 0) {
	...
	n = fp-genbuf; 	if(write(io, genbuf, n) != n) { puts(WRERR); error(Q); }
}
...
char * _getline(unsigned int tl) { char *bp, *lp; int nl; lp = linebuf; bp = getblock(tl, READ); nl = nleft; tl &= ~((BLKSIZE/2)-1);
	while (*lp++ = *bp++)
...
char * getblock(unsigned int atl, int iof) { int bno, off; bno = (atl/(BLKSIZE/2)); off = (atl<<1) & (BLKSIZE-1) & ~03;
	...
	if (iof==READ) {
		...
		ichanged = 0; iblock = bno; rblkio(bno, ibuff, read); return(ibuff+off);
	}
	if (oblock>=0) { blkio(oblock, obuff, write); }
	oblock = bno; return(obuff+off);
}
void blkio(int b, char *buf, ssize_t (*iofcn)(int, const void*, size_t)) {
...
void cerror(void) { expbuf[0] = 0;  nbra = 0;  error(Q); }
...
void putd(void) { int r = count%10; count /= 10; if (count) { putd(); } putchr(r + '0'); }
void putchr(int ac) { char *lp = linp; int c = ac;
	if (listf) {
		if (c=='\n') {
