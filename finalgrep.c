...
void search(const char* word) {
  ...
  char* genbufp = genbuf;
  char* c;
  while(genbufp != NULL && *genbufp != '\0') {
    c = genbufp;
    genbufp = strchr(genbufp + 1, '\n');
    if(genbufp != NULL) {
      ++genbufp;
      memcpy(linebuf, c, genbufp - c);
      linebuf[genbufp - c] = '\0';
      if(strstr(linebuf, word) != NULL) {
          printf("%s", linebuf);
    }
  }
}
...
void readfile(const char* searchFile) {
  filename(searchFile);
  init();
  if ((io = open(file, 0)) < 0) {
    lastc = '\n';
...
void exfile(void) {  close(io);  io = -1;  if (vflag) {  putd();  puts_(" characters read"); }  }
void filename(const char* fn) { int c = getchr();  strcpy(file, fn);  strcpy(savedfile, fn);  }
